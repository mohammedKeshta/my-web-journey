// Copyright 2020 Mohammed Elzanaty - https://bit.ly/2Zf1RFN . All rights reserved.
...
function checkActive() {
  chrome.tabs.getSelected(null, function (tab) {
    ...
    chrome.tabs.sendMessage(tab.id, { action: "getStatus" }, function (
      isActive
    ) {
    ...
    chrome.tabs.sendMessage(tab.id, { action: "toggle" }, function (response) {
      ...
      if (!response) {
        chrome.tabs.executeScript(tab.id, {
          code:
            "if (confirm('This tab was loaded before CTRE was installed. Would you like to reload it?\\nThis is necessary only the first time.')) location.reload();",
        });
      }
    });
  });
